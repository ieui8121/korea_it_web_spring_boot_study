...
//서버 사이드 렌더링은 서버측에서 프론트의 웹페이지 까지 통째로 반환 그걸 부라우저에서 띄움
//그럼 해당 요청 경로에 따라서 웹피이지가 다 할당되어있어서 느림

//하지만 우리가 하게 될 프론트(리엑트) + 벡엔드(스프링부트 웹서버)
//리엑트는 클라이언트 사이드 렌더링(CSR) 즉 프론트엔드 쪽에서 웹피이지를 로드
//벡엔드는 요청에 해당하는 데이터만 반환시켜줌
...
public class PostController {
    ...
    //필요성 객체(의존성)를 직접 만들지 않고, 외부(스프링부트)에서 대신 넣어주는 것
